...
# APIs (as chaves agora vêm de variáveis de ambiente)
...
CMC_API_KEY = os.getenv("CMC_API_KEY", "")  # Chave agora vem de variável de ambiente
...
def is_valid_bitcoin_address(address):
    ...
    # Verificação básica de regex
    if not re.match(r'^(bc1|[13])[a-zA-HJ-NP-Z0-9]{25,39}$', address):
        return False
    
    # Verificação adicional (opcional) - checksum para endereços legacy
    if address.startswith('1') or address.startswith('3'):
        # Implementar verificação de checksum aqui
        pass
...
def get_bitcoin_price_with_cache(api="all"):
    ...
    current_time = time.time() // (60 * CACHE_TTL_MINUTES)  # Cache expira a cada CACHE_TTL_MINUTES
...
def mostrar_precos():
    ...
    with tqdm(total=3, desc="Consultando APIs", unit="API") as pbar:
        ...
        # Adiciona pequeno delay para melhor visualização
        time.sleep(0.5)
        pbar.update(1)
...
def monitorar_transacao_tempo_real():
    ...
    # Thread para executar o WebSocket
