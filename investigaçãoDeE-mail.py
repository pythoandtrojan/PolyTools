...
from functools import lru_cache, wraps # Importe lru_cache e wraps
import hashlib
...
# Decorator para controle de rate limit
def rate_limit(calls_per_second: int):
    min_interval = 1.0 / calls_per_second
    last_call_time = {} # Armazena o último tempo de chamada por função
...
class Config:
    ...
    # APIs gratuitas e outras APIs para integração (URLs base)
    APIS = {
        ...
        'telegram_check': 'https://my.telegram.org/auth/send_password', # Não oficial, instável
    ...
    # Chaves de API (obtenha as suas e defina como variáveis de ambiente!)
    API_KEYS: Dict[str, str] = {
        ...
        # Adicione outras chaves conforme necessário (e.g., SOCIALBLADE_API_KEY)
    }
...
class TermuxUtils:
    ...
    def check_dependencies():
        ...
        required_packages = [
            ...
            "bs4", # BeautifulSoup
            "diskcache",
            # "google", # Para ForensicAnalyzer.estimate_account_age, se for usar
            # "python-dotenv" # Para carregar .env, se for usar (não implementado aqui)
        ]
...
class GmailInvestigator:
    ...
    @rate_limit(calls_per_second=0.5) # Limite para evitar bloqueios do Google
    ...
    def check_breaches(self, email: str) -> Dict:
        ...
        sources = {
            ...
            'telegram': self._check_telegram_account, # Aviso: instável e não oficial
            'leakcheck': self._check_leakcheck,
            'hunter_io': self._check_hunter_io,
            'emailrep': self._check_emailrep,
            'gravatar': self._check_gravatar,
            'dehashed': self._check_dehashed, # Nova
            'leaklookup': self._check_leaklookup # Nova
            # Adicione mais aqui conforme implementar: 'socialblade', 'ipqualityscore', etc.
        }
    ...
    @rate_limit(calls_per_second=1) # Exemplo: 1 chamada por segundo
    def _check_breachdirectory(self, email: str) -> Dict:
        """Verifica vazamentos usando BreachDirectory."""
        # BreachDirectory tem um plano "Free" no RapidAPI, mas é limitado
        # Este endpoint 'free' pode ter limites rigorosos ou ser descontinuado.
        ...
        try:
            headers = {
                'X-RapidAPI-Key': 'free', # Use sua chave RapidAPI real aqui para melhor desempenho
    ...
    @rate_limit(calls_per_second=0.5) # Limite para Snusbase (público)
    def _check_snusbase(self, email: str) -> Dict:
        ...
        try:
            response = requests.post(
                ...
                auth=('public', ''), # Snusbase oferece um endpoint público limitado
    ...
    @rate_limit(calls_per_second=0.2) # Esta verificação é mais sensível
    def _check_telegram_account(self, email: str) -> Dict:
        ...
        try:
            # O Telegram usa principalmente números de telefone. Tentar verificar via email
            # é uma heurística fraca baseada em endpoints não documentados ou comportamento de formulários.
            # O campo 'phone' no endpoint 'send_password' geralmente espera um número de telefone.
            # Estamos usando o email aqui na esperança de alguma validação de formato que revele existência.
            response = requests.post(
                Config.APIS['telegram_check'],
                data={'phone': email}, # Idealmente, deveria ser um número de telefone
                timeout=Config.TIMEOUT,
                headers={'User-Agent': Config.USER_AGENT}
            )
            
            # Heurística: 'phone_num_unconfirmed' ou 'auth_code_sent' podem indicar que o "número" (email) é reconhecido.
    ...
    @rate_limit(calls_per_second=0.5) # Limite para LeakCheck.io
    ...
    @rate_limit(calls_per_second=0.5) # Limite para EmailRep.io
    def _check_emailrep(self, email: str) -> Dict:
        ...
        try:
            # EmailRep.io não requer API Key para consultas básicas, mas tem limites de taxa.
            headers = {'User-Agent': Config.USER_AGENT}
            response = requests.get(
    ...
    @rate_limit(calls_per_second=1) # Gravatar é bem tolerante a requisições
    def _check_gravatar(self, email: str) -> Dict:
        """Verifica se o email tem um perfil Gravatar."""
        email_hash = hashlib.md5(email.lower().encode('utf-8')).hexdigest()
        gravatar_url = f"https://www.gravatar.com/avatar/{email_hash}?d=404" # d=404 retorna 404 se não existir
        
        cache_key = f"gravatar_{email_hash}"
        cached = self.cache.get(cache_key)
        if cached: # cached já conterá o dicionário padronizado
    ...
    @rate_limit(calls_per_second=0.2) # Ajuste conforme o limite da API do Dehashed
    def _check_dehashed(self, email: str) -> Dict:
        ...
        try:
            # A autenticação do Dehashed geralmente usa Basic Auth: username (email) e password (API Key)
            headers = {'User-Agent': Config.USER_AGENT}
    ...
    @rate_limit(calls_per_second=0.2) # Ajuste conforme o limite da API do Leak-Lookup
    ...
    def find_related_accounts(self, email: str) -> List[str]:
        ...
        # Variações comuns para Gmail
        variations = [
            f"{username}{i}@gmail.com" for i in range(1, 3)
        ] + [
            f"{username}.{i}@gmail.com" for i in range(1, 3)
        ]
        # Outros domínios comuns para o mesmo username
        common_domains = [
            f"{username}@outlook.com",
            f"{username}@hotmail.com",
            f"{username}@yahoo.com"
        ]
        return list(set(variations + common_domains)) # Usa set para remover duplicatas
        ...
        try:
            ...
            for url in results:
                try:
                    ...
                    for year in range(2004, datetime.now().year + 1): # Gmail lançado em 2004
                        if str(year) in text:
...
class RiskAssessor:
    ...
    def calculate_risk(leaks: Dict, breaches: Dict) -> Dict:
        ...
        # Leaks do Google Password Checkup
        ...
        # Vazamentos gerais (BreachDirectory, Snusbase, LeakCheck, Dehashed, LeakLookup)
        ...
        if total_breach_entries > 0:
            score += min(total_breach_entries * 2, 30) # Pontuação por entradas, limitado a 30
            
        # Hunter.io (verificação profissional e descartável)
        ...
        if hunter_data and hunter_data.get('found'):
            ...
            # Exemplo de risco baseado no score de qualidade do Hunter.io (se aplicável)
        ...
        # EmailRep.io (reputação e atividades maliciosas)
        ...
        # Telegram account (não oficial, apenas um sinal leve)
        telegram_data = breaches.get('telegram')
        if telegram_data and telegram_data.get('found'):
            score += 5 # Peso baixo devido à instabilidade e natureza não oficial
        ...
        # Gravatar presence (indicador de uso público)
        gravatar_data = breaches.get('gravatar')
        if gravatar_data and gravatar_data.get('found'):
            score += 2 # Peso muito baixo, apenas indica que o email é usado publicamente
...
class ReportGenerator:
    ...
    def generate_report(email: str, investigator: GmailInvestigator) -> Dict:
        ...
        # A estimativa de idade é experimental, descomente se quiser ativar
        # logging.info(f"{Fore.BLUE}[*] Estimando idade da conta (pode demorar e ser impreciso)...{Style.RESET_ALL}")
        # account_age = ForensicAnalyzer.estimate_account_age(email)
        ...
        # if account_age:
        #     report_data['account_age_estimate'] = account_age
    ...
    def print_termux_report(report: Dict):
        ...
        # Verificação de Contas/Serviços
...
def main():
    try:
        # Configuração inicial para Termux
        if "com.termux" in os.getcwd():
        ...
        if "com.termux" in os.getcwd():
            ...
            # Para desktop, usar pprint para melhor formatação de dicionários complexos
