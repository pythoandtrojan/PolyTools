...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .title { font-size: 2.5rem; color: #00ff00; text-shadow: 0 0 10px #00ff00; margin-bottom: 10px; }
        .subtitle { font-size: 1.2rem; color: #00ff88; opacity: 0.8; }
        .main-content { display: grid; grid-template-columns: 1fr 1fr; gap: 30px; margin-bottom: 30px; }
        @media (max-width: 768px) { .main-content { grid-template-columns: 1fr; } }
        .terminal { 
            background: #000000; border: 2px solid #00ff00; border-radius: 10px; 
            padding: 20px; height: 400px; overflow-y: auto; box-shadow: 0 0 20px rgba(0, 255, 0, 0.3);
        }
        .terminal-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px; padding-bottom: 10px; border-bottom: 1px solid #00ff00; }
        .terminal-title { font-size: 1.2rem; color: #00ff00; }
        .terminal-controls { display: flex; gap: 10px; }
        .control-btn { width: 12px; height: 12px; border-radius: 50%; border: none; cursor: pointer; }
        .close { background: #ff5f56; } .minimize { background: #ffbd2e; } .maximize { background: #27ca3f; }
        .terminal-body { font-family: 'Courier New', monospace; line-height: 1.4; }
        .prompt { color: #00ff00; } .command { color: #ffffff; } .output { color: #00ff88; margin: 5px 0; }
        .error { color: #ff4444; } .success { color: #00ff00; }
        .upload-section { background: rgba(0, 0, 0, 0.7); border: 2px solid #00ff00; border-radius: 10px; padding: 25px; box-shadow: 0 0 20px rgba(0, 255, 0, 0.3); }
        .upload-title { font-size: 1.5rem; color: #00ff00; margin-bottom: 20px; text-align: center; }
        .file-input { width: 100%; padding: 15px; margin-bottom: 20px; background: rgba(0, 255, 0, 0.1); border: 1px solid #00ff00; border-radius: 5px; color: #ffffff; font-family: 'Courier New', monospace; }
        .file-input::file-selector-button { background: #00ff00; color: #000000; border: none; padding: 8px 15px; border-radius: 3px; cursor: pointer; font-family: 'Courier New', monospace; font-weight: bold; }
        .compress-btn { width: 100%; padding: 15px; background: #00ff00; color: #000000; border: none; border-radius: 5px; font-size: 1.1rem; font-weight: bold; cursor: pointer; transition: all 0.3s ease; font-family: 'Courier New', monospace; }
        .compress-btn:hover { background: #00cc00; box-shadow: 0 0 15px rgba(0, 255, 0, 0.5); }
        .compress-btn:disabled { background: #666666; cursor: not-allowed; }
        .features { display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 20px; margin-top: 40px; }
        .feature-card { background: rgba(0, 0, 0, 0.7); border: 1px solid #00ff00; border-radius: 8px; padding: 20px; text-align: center; transition: transform 0.3s ease; }
        .feature-card:hover { transform: translateY(-5px); box-shadow: 0 5px 15px rgba(0, 255, 0, 0.3); }
        .feature-icon { font-size: 2rem; margin-bottom: 10px; }
        .feature-title { font-size: 1.2rem; color: #00ff00; margin-bottom: 10px; }
        .feature-desc { color: #00ff88; font-size: 0.9rem; }
        .footer { text-align: center; padding: 20px; margin-top: 40px; border-top: 1px solid #00ff00; color: #00ff88; opacity: 0.7; }
        .cursor { display: inline-block; width: 8px; height: 16px; background: #00ff00; margin-left: 5px; animation: blink 1s infinite; }
        @keyframes blink { 0%, 100% { opacity: 1; } 50% { opacity: 0; } }
        .progress-container { width: 100%; height: 20px; background: rgba(0, 255, 0, 0.1); border-radius: 10px; margin: 20px 0; overflow: hidden; }
        .progress-bar { height: 100%; background: linear-gradient(90deg, #00ff00, #00ff88); border-radius: 10px; transition: width 0.3s ease; width: 0%; }
        .download-section { text-align: center; margin-top: 20px; padding: 15px; background: rgba(0, 255, 0, 0.1); border-radius: 5px; display: none; }
        .download-btn { display: inline-block; padding: 12px 25px; background: #00ff00; color: #000000; text-decoration: none; border-radius: 5px; font-weight: bold; transition: all 0.3s ease; }
        .download-btn:hover { background: #00cc00; box-shadow: 0 0 15px rgba(0, 255, 0, 0.5); }
    </style>
</head>
<body>
    <div class="container">
        ...
        <div class="main-content">
            ...
            <div class="upload-section">
                ...
                <button class="compress-btn" id="compressBtn" onclick="compressFiles()">INICIAR COMPRESSÃO</button>
        ...
        <div class="features">
            <div class="feature-card"><div class="feature-icon">⚡</div><h3 class="feature-title">COMPRESSÃO RÁPIDA</h3><p class="feature-desc">Algoritmos otimizados para máxima velocidade</p></div>
            <div class="feature-card"><div class="feature-icon">🔒</div><h3 class="feature-title">SEGURANÇA</h3><p class="feature-desc">Seus arquivos são processados localmente</p></div>
            <div class="feature-card"><div class="feature-icon">📊</div><h3 class="feature-title">ALTA QUALIDADE</h3><p class="feature-desc">Mantém a qualidade com tamanho reduzido</p></div>
            <div class="feature-card"><div class="feature-icon">🆓</div><h3 class="feature-title">GRATUITO</h3><p class="feature-desc">Sem custos ou limitações de uso</p></div>
        </div>
        ...
        function compressFiles() {
            ...
            const interval = setInterval(() => {
                ...
                if (progress >= 100) {
                    ...
                    const blob = new Blob(['Simulated compressed PDF content'], { type: 'application/pdf' });
                    const url = URL.createObjectURL(blob);
                    downloadLink.href = url;
                    downloadLink.download = 'arquivo_compactado.pdf';
                    addTerminalOutput('success', '[SISTEMA] Compressão concluída com sucesso!');
                    addTerminalOutput('output', '[RESULTADO] Arquivo compactado: arquivo_compactado.pdf (' + (Math.floor(Math.random() * 500) + 100) + 'KB)');
                    setTimeout(() => {
...
{Colors.YELLOW}Arquivo:{Colors.RESET} index.html
